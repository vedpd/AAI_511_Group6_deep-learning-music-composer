"""
Training script for CNN composer classification model.
"""
...
from src.utils.config import (CNN_BATCH_SIZE, CNN_EPOCHS, CNN_EARLY_STOPPING_PATIENCE,
                           BEST_CNN_MODEL, FINAL_CNN_MODEL, CHECKPOINTS_DIR,
                           COMPOSERS, RANDOM_SEED)
from src.utils.helpers import set_random_seed, format_time, count_parameters
from src.models.cnn_model import CNNComposerClassifier


class CNNTrainer:
    """
    Trainer for CNN composer classification model.
    """
    
    def __init__(self,
                 model: CNNComposerClassifier,
    ...
    def train(self,
        ...
        if checkpoint_path is None:
            checkpoint_path = BEST_CNN_MODEL
        ...
        print(f"Training CNN model for up to {self.epochs} epochs...")
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray) -> Dict[str, float]:
        ...
        # Load best model
        if BEST_CNN_MODEL.exists():
            print(f"Loading best model from {BEST_CNN_MODEL}")
            self.model.model = keras.models.load_model(BEST_CNN_MODEL)
    ...
    def save_final_model(self, output_path: Optional[Path] = None) -> None:
        ...
        if BEST_CNN_MODEL.exists():
            # Save the best model
            import shutil
            shutil.copy(BEST_CNN_MODEL, output_path)
...
def train_cnn_model(X_train: np.ndarray,
                   y_train: np.ndarray,
                   X_val: np.ndarray,
                   y_val: np.ndarray,
                   X_test: np.ndarray,
    ...
    """
    Convenience function to train CNN model with standard parameters.
    ...
    # Create trainer
    trainer = CNNTrainer(model, **kwargs)
